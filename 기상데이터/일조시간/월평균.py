...
for stn_id in station_ids:
    ...
    try:
        ...
        if df is not None and not df.empty:
            ...
            # 일조시간 컬럼명 통일
            possible_cols = ['일조시간', '일조시간(시간)', 'SUNSHINE', 'SUNSHINE_HR']
            for c in possible_cols:
                if c in df.columns:
                    df = df.rename(columns={c: '일조시간'})
                    break
...
weather_df['일조시간'] = pd.to_numeric(weather_df['일조시간'], errors='coerce')
...
print("\n✅ 시도별 월평균 일조시간 계산 완료 (단위: 시간)")
print(region_monthly_df.head(15))
region_monthly_df.to_csv('region_monthly_sunshine.csv', encoding='utf-8-sig')
print("\n💾 'region_monthly_sunshine.csv' 저장 완료!")
